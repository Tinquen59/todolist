...
export const addNewTask = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!body.description || !body.userId) return res.status(401).json({ error: "Empty fields !!"});
...
export const detailTaskController = async(req: Request, res: Response) => {
    try {
        ...
        const findDetailTask = await Task.findById(new mongoose.Types.ObjectId(req.params.id), projection);

        if (!findDetailTask) return res.status(500).json({ error : "No task found" });
...
export const updateTaskController = async (req: Request, res: Response) => {
    const { body } = req;
    try {
        if (body.description === "" || !req.params.id) return res.status(401).json({ error: "Empty fields !!" });
...
export const removeTaskController = async(req: Request, res: Response) => {
    try {
        ...
        if (deleteTask.deletedCount === 0) return res.status(500).json({ error: "Error ! Filter does not match" })
